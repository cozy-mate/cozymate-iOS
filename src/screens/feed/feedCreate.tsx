...
// const POST_UPDATE_ERROR = '게시글 수정에 실패했습니다.';
...
//     error,
//   } = useGetDetailPostForEdit(roomInfo.roomId, postId!);
...
//     if (mode === 'edit') {
//       if (getPostSuccess) {
//         setImages(postDetailData?.images!);
//         setPostDescription(postDetailData?.postDescription!);
//       }
//       if (getPostError) {
//         //ERROR에 따라 변동되는 Modal 보여줘야 함.
//         setModalTitle(POST_GET_ERROR);
...
//   const handleUploadImages = (): Promise<string[]> => {
//     return new Promise((resolve, reject) => {
//       if (images.length > 0) {
//         uploadImages(images, {
//           onSuccess: (data) => resolve(data.imgUrlList as string[]),
//           onError: (error) => reject(error),
//         });
//       } else {
//         resolve([]); // 이미지가 없으면 빈 배열 반환
//       }
//     });
//   };

//   const handleCreatePost = (imageUrls: string[]) => {
//     return new Promise((resolve, reject) => {
//       createPost(
//         {
...
//       // 포스트 생성
//       await handleCreatePost(imageUrls);
...
//       setModalTitle(error === uploadImageError ? IMAGE_UPLOAD_ERROR : POST_CREATE_ERROR);
//       handleButtonModalOpen();
//     }
//   };
...
//     try {
//       if (images.length > 0) {
//         // Await the image upload to get URLs
//         const response = await uploadImages(images);
//         imageResponse = { imgUrlList: response.urls }; // Assuming `response.urls` contains the uploaded image URLs
//       }
//     } catch (e) {
//       console.log(e);
...
//       setModalTitle(POST_UPDATE_ERROR);
//       handleButtonModalOpen();
//     }
//   }, [images, postDescription, postId, roomInfo, setModalTitle, setIsGoingBack, handleButtonModalOpen]);

//   const handleSubmit = useCallback(() => {
//     if (createPostPending || updatePostPending || uploadImagePending) return;
//     if (mode === 'edit') {
//       updateMyPost();
//     }
// <<<<<<< Updated upstream
...
//       createMyPost();
//     }
//   }, [createPostPending, updatePostPending, uploadImagePending, mode, createMyPost, updateMyPost]);
// >>>>>>> Stashed changes
...
//   return (
//     <>
//       {getPostLoading || createPostPending || uploadImagePending || updatePostPending && <LoadingComponent />}
...
//           <View className="flex">
//             <Pressable
//               onPress={isComplete ? handleSubmit : () => { }}
