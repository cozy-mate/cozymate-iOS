...
const FeedEditScreen = (props: FeedEditScreenProps) => {
  ...
  const [, setNeedsRefresh] = useRecoilState(feedRefreshState);
  ...
  const getFeedInfo = async () => {
    try {
      setIsLoading(true);
      const response = await getFeedData(myRoom.roomId);
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <SafeAreaView className="h-full w-full flex-1 flex-col bg-white px-8 pt-1">
        ...
        <View className="flex-1 flex-col">
          <Text className="mb-2 ml-1 text-base font-semibold text-emphasizedFont">
            피드 이름을 입력해주세요.
          </Text>
          <TextInput
